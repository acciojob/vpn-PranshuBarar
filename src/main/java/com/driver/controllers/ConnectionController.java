...
public class ConnectionController {
    ...
    public ResponseEntity<Void> connect(@RequestParam int userId, @RequestParam String countryName) throws Exception{
        ...
        //1. If the user is already connected to any service provider, "Already connected" exception will be thrown.
        //2. Else if the countryName corresponds to the original country of the user, nothing will be done.
        // This means that the user wants to connect to its original country, for which we do not require a connection.
        // Thus, returning the user as it is.
        //3. Else, the user should be subscribed under a serviceProvider having option to connect to the given country.
            //If the connection can not be made. As user does not have a serviceProvider or serviceProvider does not have given country, "Unable to connect" exception will be thrown.
            //Else, we will establish the connection where the maskedIp is "updatedCountryCode.serviceProviderId.userId" and updated user will be returned.
        // If multiple service providers allow user to connect to the country, user will use the service provider having smallest id.
    ...
    public ResponseEntity<Void> disconnect(@RequestParam int userId) throws Exception{
        //If the given user was not connected to a vpn, "Already disconnected" exception will be thrown.
        //Else, he/she will be disconnected from vpn, we will make maskedIp as null, update relevant attributes and return updated user.
    ...
    public ResponseEntity<Void> communicate(@RequestParam int senderId, @RequestParam int receiverId) throws Exception{
        //Establishing a connection between sender and receiver users
        ...
        //If the sender's original country matches receiver's current country, we do not need to do anything as they can communicate. We will return the sender as it is.
        //If communication can not be established due to any reason, "Cannot establish communication" exception will be thrown.
